<template>
  <div class="flex row justify-center" style="height:80%; width:100%">
    <div class="flex row justify-center" style="width: 100%;margin: 32px 0px;">
      <button @click="getAverage()">Test Ocupacional</button>
    </div>
...
export default {
    ...
    return {
      ...
      options: {
        ...
        title: {
          ...
          fontSize: 18,
          text:
            "                                                Resultado Global de Competencias"
        },
  ...
  created() {
    ...
    //   datos_tecnicos
    // ];
    // let notas = datos_totales.map(
    //   x => x.map(
    //     y => parseInt(y.nota_competencia)
    //   )
    // )
    // let count_notas = notas.flat();
    // let categories_notas = {
    //   destacado: [],
    //   bueno: [],
    //   necesita_mejorar: [],
    //   debajo_de_lo_esperado: []
    // }
    // for (let j = 0; j <= count_notas.length - 1; j++) {
    //   if(count_notas[j] == 19) {
    //     categories_notas.destacado.push(count_notas[j])
    //   }
    //   else if(count_notas[j] == 16) {
    //     categories_notas.bueno.push(count_notas[j])
    //   }
    //   else if(count_notas[j] == 13) {
    //     categories_notas.necesita_mejorar.push(count_notas[j])
    //   }
    //   else {
    //     categories_notas.debajo_de_lo_esperado.push(count_notas[j])
    //   }
    // }
    // console.log(categories_notas);
  },
  methods: {
    getAverage() {
      ...
      for (let j = 0; j <= labels_competencias.length - 1; j++) {
        suma_total = 0;
        cantidad_total = 0;
        for (let i = 0; i <= datos_totales.length - 1; i++) {
          if (
            Object.keys(datos_totales[i]).findIndex(
              element => element == labels_competencias[j]
            ) !== -1
          ) {
            suma_total =
              suma_total +
              datos_totales[i][labels_competencias[j]] *
                datos_totales[i].cantidad;
            cantidad_total = cantidad_total + datos_totales[i].cantidad;
          }
        }
        this.results_avarage.push((suma_total / cantidad_total).toFixed(1));
      }
    },
    fillData() {
      this.datacollection = {
